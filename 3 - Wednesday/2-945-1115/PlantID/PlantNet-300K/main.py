...
def train(args):
    ...
    for epoch in tqdm(range(args.n_epochs), desc='epoch', position=0):
        ...
        train_acc.append(acc_epoch_train)
        #Once the training of the epoch is complete, do the validation on that epoch 
        loss_epoch_val, acc_epoch_val, topk_acc_epoch_val, avgk_acc_epoch_val, lmbda_val = val_epoch(model, val_loader, criteria,
                                                                                                     loss_val, acc_val, topk_acc_val, avgk_acc_val,
                                                                                                     class_acc_val, args.k, dataset_attributes, args.use_gpu)
        val_acc.append(acc_epoch_val)
    ...
    loss_test_ba, acc_test_ba, topk_acc_test_ba, avgk_acc_test_ba, class_acc_test = test_epoch(model, test_loader, criteria, args.k,
